...
export function composeInviteReport({ person, link, primaryOk, fallbackOk, siteOk, authorEmail, fromEmail }) {
  ...
  const lines = [
    ...
    ...(pass
      ? ["链接可以发给朋友了，直接转发：", link]
      // 措辞必须与实际行为一致：邮件一旦发出（无论自检过没过），该 token 就进了「已见」，
      // 下个 tick 不会再复检重发。原文承诺「修好后自动复检重发」是假的，会让作者干等。
      : ["请先排查（Worker 部署 / KV / 站点部署）。注意：本条已记入「已见」，修好后**不会**自动复检重发；",
         "确认修好后请手动验证该链接可用，或用管理页对这个人 rotate 一次换新 token 触发重新自检。"]),
    "",
