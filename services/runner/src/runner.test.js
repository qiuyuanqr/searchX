...
test("补发成功后立即落盘：阶段 2 取 Issue 列表因外部瞬时故障失败，已补发条目也不会留在队列里重复发信", async () => {
  const saves = [];
  const fetchImpl = async (url, opts = {}) => {
...
test("拉 approved 队列 fetch 抛错（纯网络不可达）→ summary.queueUnreachable=true 且不抛，交由防抖判定是否报警", async () => {
  // 模拟 withNetRetry 重试用尽后 rethrow 的 DOMException(TimeoutError)：墙内到 api.github.com 的分钟级瞬断
  const fetchImpl = async (url) => {
    if (String(url).includes("/issues?")) throw Object.assign(new Error("The operation timed out."), { code: 23, name: "TimeoutError" });
    return { ok: false, status: 404, json: async () => ({}) };
  };
  let ran = 0;
  const summary = await runOnce(CONFIG, {
    fetchImpl, scanDirs: () => [], runResearch: async () => { ran++; return true; },
    sendEmail: async () => {}, log: () => {},
  });
  expect(summary.queueUnreachable).toBe(true);
  expect(summary.processed).toBe(0); // 拉不到队列 → 本轮不处理任何 Issue
  expect(ran).toBe(0);               // 绝不因拉队列失败而空跑研究
});

test("拉 approved 队列 GitHub 5xx（服务端抽风返回错误页）→ queueUnreachable，同样防抖不立即报警", async () => {
  const fetchImpl = async (url) => {
    if (String(url).includes("/issues?")) return { ok: false, status: 503, text: async () => "<html>GitHub error</html>" };
    return { ok: false, status: 404, json: async () => ({}) };
  };
  const summary = await runOnce(CONFIG, {
    fetchImpl, scanDirs: () => [], runResearch: async () => true,
...
test("拉 approved 队列 4xx（如 401 PAT 失效）→ 抛错，不防抖：配置问题须立即报警、不能被防抖掩盖", async () => {
  const fetchImpl = async (url) => {
    if (String(url).includes("/issues?")) return { ok: false, status: 401, text: async () => "Bad credentials" };
    return { ok: false, status: 404, json: async () => ({}) };
  };
