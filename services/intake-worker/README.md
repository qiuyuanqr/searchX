...
Worker 不随 CI 自动上线，改完 `src/` 本来得手动 `wrangler deploy`——容易忘、也烦。MacBook 上装了个 LaunchAgent 每小时兜底：检测 `src/`、`wrangler.toml` 自上次部署以来有变化就自动 `wrangler deploy`，没变秒退、不刷无意义 version。所以日常改完 worker 只要 commit + push，最多一小时内自动上线（想立刻上线仍可手动 `bun x wrangler deploy`）。

- 两件套：`deploy-cron.sh`（刻意不主动 `git pull`——靠双机同步保 HEAD 最新，各司其职）+ `launchd/com.searchx.worker-deploy.plist`（`StartInterval=3600`、`RunAtLoad=false`）。
- **只在 MacBook 装**（它的 wrangler 已登录）；Mac mini 同步到脚本但没装 plist、不会跑，避免双机重复部署。
- 安装：
  ```bash
  ...
  ```
- 排障：看 `~/Library/Logs/searchx-worker-deploy/worker-deploy.log`；想立刻触发一次 `launchctl kickstart -k "gui/$(id -u)/com.searchx.worker-deploy"`；卸载 `launchctl bootout "gui/$(id -u)/com.searchx.worker-deploy"`。
