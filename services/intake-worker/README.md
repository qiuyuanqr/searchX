# searchX Intake Worker（M2a · 入队闭环）

唯一公开**写**入口：站内提交表单 POST 到这个 Cloudflare Worker，Worker 校验后在 GitHub 建一条 `pending` Issue 并通知作者。**全程 0 花费、0 上线**——花 Claude 额度的只有后续 M2b 跑 `/research`。
...
Cloudflare Worker (本目录)
   ...
   └─ INTAKE_KV.put(`sub:<issue号>`, 真实邮箱)   ← 私有存，留给 M2b Emailer
   ▼
...
- **仓库是公开的，Issue 也公开。** 提交者的**真实邮箱绝不进 Issue 正文**——`formatIssue` 内部调 `maskEmail`，Issue 里只出现 `a****@gmail.com`。
...
| `src/issue-format.js` | `formatIssue(clean,{author})` + `maskEmail(email)` 拼 Issue（用代码围栏防 markdown 注入） |
...
| `src/handler.js` | `handleIntake(request,env,deps)` 编排（CORS/方法/校验/限频/建 Issue/存邮箱） |
...
```bash
bun run build:worker   # 在仓库根目录跑，产出 services/intake-worker/dist/worker.js
```
Workers & Pages → Create Worker → 编辑器粘贴 `dist/worker.js` 全文 → Settings：
...
5. 全程**无 `/research` 运行、无站点内容变更**（0 花费、0 上线新报告）。
6. 反向：连发超过每日上限 → 返回限频文案。
7. 清理：关掉测试 Issue。
