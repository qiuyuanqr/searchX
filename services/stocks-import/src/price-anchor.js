...
const TRIGGER = "突破|跌破|失守|站上|站稳|回落至|回落到|下探至|上探至|回踩至|回踩|回调至|回测|测试|止损|止盈";
// 相对锚：可复现的客观刻度。没有这些词就是裸价位，不动。
// ⚠️ 词表要覆盖**实际写法**，不是概念名。2026-09-01 两篇（002903 / 300583）被搁置的根因
// 就在这里：Stocks 侧完全按它自己的 SKILL 写了「锚在前、数值紧跟其后」，但写的是
// 「MA20 29.24 元」「9-01 当日最低 13.81 元」「8-31 收盘 12.97 元」——表里只有「均线」
// 「低点」这些**概念名**，认不出 `MA20`、「最低」、「收盘」这些**人真正会写的形式**，
// 于是有锚也当成裸价位放过，全被 QC 拦下搁置。加词时照着真实报告原文加，别照着术语表加。
// 2026-09-18 同一个坑又犯一次：茂莱光学 688502 写的是「跌破茂莱转债转股价 364.15 元」——
// 转股价是可转债公告里的客观刻度、完全合规的锚，表里没有，于是又被当成裸价位搁置。
// 补进转债那一族（转股价 / 赎回触发线）。**加词必须同时更新 Stocks 侧 SKILL 的锚清单**，
// 那份清单是写作者唯一看得到的凭据，只加这边等于承诺「认得」却仍认不出。
const ANCHOR = "成本|分位|成本线|均本|中枢|均线|MA\\d+|支撑位|压力位|低点|高点|前低|前高|最低|最高|收盘|开盘|起点|授予价|回购上限|转股价|转股价格|赎回触发线|强赎触发线";
