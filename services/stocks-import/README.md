...
## 三个标签维度（src/sectors.js）

报头上有三行标签，**性质不同，刻意分开陈述**：

| 维度 | 来源 | 怎么定 |
|---|---|---|
| 关联板块 | `src/mapping.js` 手工表 | 作者自己的五大关注框架，人工判断 |
| 申万行业 | 库表 `stock_industry` + `industry_sw` | 客观唯一，直接读 |
| 概念标签 | 库表 `theme` + `theme_stock` | 剔噪音后按特异度排序取前 8 |

两张库表都在 stock SKILL §2.3 的白名单内。三个坑记在这里：

- **ts_code 两格式**：`research_report` / `stock_basic` 存 6 位裸码，而 `stock_industry` /
  `theme_stock` 存**带后缀**（`688635.SH`）。不拼后缀就静默查空、表现成「这票没有行业」。
- **申万只能取到二级**：库里 L2 的 `parent_code` 用 `110000` 编码、L1 的 `industry_code` 是
  `801010.SI`，134 个 L2 里 0 个连得上 L1。所以没有「电子 → 通信设备」这种路径。
- **覆盖率 92.5%**（5441/5882）：新股常缺（长进光子就没有）。缺就不显示，不编造。

### 概念标签为什么不能直接当板块用

同花顺概念一只票挂 7–68 个。**实测反证**：泓博医药是 CRO 医药公司，同花顺给它挂着
「AI应用」「人工智能」「ChatGPT概念」「多模态AI」——照抄就会挂上 AI应用 板块，
而正确答案是不挂任何板块。所以概念只用来给**建议**，打在导入日志里等人确认，
永不自动写进 `mapping.js`。

噪音判据按**类别**剔除（交易机制 / 指数成分 / 资讯商榜单 / 持股方 / 上市状态 /
资本行为 / 财报事件 / 汇率联动 / 股权性质 / 注册地），拿库里 899 个真概念标定过：
剔除 48 个、逐条核对无误伤。**不能只靠成分股数量当阈值**——沪深300样本股 319 只、
次新股 167 只是纯噪音但数量小，而人工智能 1085 只数量大却是真业务概念，两头都会错。
剩下的按成分股数升序排（光纤概念 112 比人工智能 1085 更能说明公司干什么），泛概念自然沉底。

板块建议**既会多挂也会漏挂**，日志里写明了这一点：概念泛化时带出无关板块；
而海光信息这类标的的标签里根本没有「算力」字样，必然漏报。判断权在人。

### 存量回填

```bash
bun run scripts/backfill-sectors.js --dry-run   # 先看会改哪些
bun run scripts/backfill-sectors.js            # 真改（幂等，按当前库值覆盖）
```

2026-08-17 已对存量 64 篇跑过一次。行业归属与概念标签本身会变，重跑即刷新。
