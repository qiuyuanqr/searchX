...
// 「情感 0.7 / 情感 +0.7 / 情感 -0.17」是 H 节事件表每行必带的字段，「新闻」覆盖正文
// 引述库内新闻标题的写法。
// ⚠️ 符号**必须可选**：第一版写成 `情感\s*[+-]`，照术语表想当然地以为正数也带「+」，
// 而真实产出里正分压倒性地不带号（2026-09-18 对存量实测：无号 25 处、带 + 29 处、带 - 13 处），
// 于是「2026-08-24「茅台股价再次站上 1300 元」（情感 0.7）」这条纯引用被当成触发条件挡下，
// 2026-09-17 茅台那篇因此被搁置。这正是「词表要照真实产出抄、别照术语表抄」——
// 而且这类失效**静默且表现为「闸太严」**，不看原文就会误以为是报告写错了。
const QUOTE_SOURCE_RE = /新闻标题|发稿时点|来源：|（来源|报道称|旧快照|库内新闻|新闻里|情感\s*[+-]?\s*\d/;
...
// 枚举式免责里的裸「无」：「全文无目标价、无评级、**无**用户持仓与账户信息」——离命中最近的
// 否定词只是一个「无」，上面那条（词表里只有「无任何」）罩不住，2026-09-07 的 300570 因此被搁置。
// ⚠️ 裸「无」**只给零间隔**，不给 14 字窗口：留了窗口，「这只票无亮点，按我的持仓成本价…」
  ...
  for (const [label, scanned] of [["报告正文", text], ["notes.md", String(notesMd || "")]]) {
    ...
    for (const { name, re } of PRIVACY_RES) {
      for (const m of scanned.matchAll(re)) {
        ...
        if (PRIVACY_BARE_NEGATION_RE.test(scanned.slice(Math.max(0, m.index - 1), m.index))) continue;
