...
// 2026-08-31 中航沈飞（600760）被误挡：「或库内研报的 2026 全年 EPS 预测中枢跌破 1.00 元」。
// EPS 是利润表指标，单位恰好也是元——但它不是股价，写进触发条件是**基本面**条件，
// 正是 §4.9 想要的那种「不给具体价位位置」的写法。豁免只放**盈利类**每股指标，判据
// 必须紧邻（不像商品那样按整句），否则同句里的股价触发条件会被蹭到豁免。
// ⚠️ 表里绝不能放「每股净资产」：「若股价跌破每股净资产 5.20 元则加仓」这类破净写法里，
// 它就是**股价的锚点**，放进来等于把真红线整条放跑（同「均价」那个坑，2026-08-25）。
test("每股盈利指标不是股价：EPS 触发条件不报（2026-08-31 中航沈飞标定）", () => {
  for (const s of [
    // 报告正文与 notes.md 的两种真实写法（一个带空格、一个不带）
    "或库内研报的 2026 全年 EPS 预测中枢跌破 1.00 元，那么回避",
    "如果单季营收<40亿或出现减值或库内2026年EPS预测中枢跌破1.00元，那么回避",
    "若扣非后每股收益跌破 0.85 元则转防御",
  ]) {
    // 变异验证：删掉 EPS_METRIC_RE 这一判据，三条全变红
    expect(checkFormat({ reportHtml: stockHtml(s), type: "股票" }).blocking).toEqual([]);
  }
});

// 守卫：EPS 豁免绝不能遮掉同句的股价触发条件——一句里两者同时出现是常态
// （同 COMMODITY_SUBJ_RE 那道「紧挨触发词前出现『股』就不放行」的守卫，只是这里
// 不能用裸「股」当判据：「每股收益」自己就带「股」字，那样写等于豁免永远不生效）。
test("守卫：同句提到 EPS，不豁免真正的股价触发条件", () => {
  for (const s of [
    "若 2026 年 EPS 中枢上修，但股价跌破 14 元则转防御",
    "如果股价跌破每股净资产 5.20 元，那么加仓",
  ]) {
    expect(checkFormat({ reportHtml: stockHtml(s), type: "股票" }).blocking.join())
      .toContain("具体触发价位");
  }
});
...
// ---- 位置式触发价位（2026-09-23：179 篇存量标定，真违规 10 篇、误报修到 0）----

const posHits = (t) => checkFormat({ reportHtml: stockHtml(t), type: "股票" }).blocking
  .filter((b) => b.includes("位置式触发价位"));

test("位置式触发价位 → 硬红线（指南针 300803 那句原话，当年规则漏过、手工改掉）", () => {
  expect(posHits("股价仍处于筹码加权平均成本 84.28 元下方，那么可小幅试仓").join()).toContain("84.28 元下方");
});

test("位置式：存量真违规的各种动词都认（词表照原文抄，只收「处于」一条都抓不到）", () => {
  for (const s of [
    "美联储 6/18 偏鸽 偏涨，挑战 50 元上方",
    "那么 可在 45 元下方分批小仓位试仓",
    "站回 250.80 元之上并伴随主力资金转为净流入（由负转正），才算资金面确认",
    "路径：题材退潮叠加筹码分散，股价下探220元以下，资金流转为全面净流出",
    "路径：多空拉锯，区间参照8-07筹码50%分位54.00元上方，等W8三季报给新证据",
    "- 试仓：6 月空调出口环比改善 + 美联储 6/18 偏鸽 → 45 元下方分批",
    "期间在 62.60 元下方的低获利盘区与 70.00 元中位成本之间反复",
  ]) expect(posHits(s).length).toBe(1);
});

test("位置式：不带价位的「处于历史低位」不报（加「处于」不会造一批误报）", () => {
  expect(posHits("股价处于历史低位，估值处于近五年 10% 分位以下")).toEqual([]);
});

test("位置式：现状陈述不报（当前价 / 现价 / 收盘作主语）", () => {
  for (const s of [
    // 「当前」离动词 24 字，过去时豁免的 20 字窗口够不着——靠同分句主语判
    "当前价 269.33 元（2026-08-14）位于 50% 成本线 241.20 元之上约 11.7%",
    "9-17 收盘 896.00 元已在 8-07 筹码 15% 分位成本 910.20 元下方——意味着多数筹码浮亏",
    "且 50% 分位成本 36.50 元恰好压在现价 36.15 元上方，意味着反弹会遇到解套盘",
  ]) expect(posHits(s)).toEqual([]);
});

test("位置式：筹码分布陈述 / 产品价格不报", () => {
  for (const s of [
    "筹码数据显示 8-07 时点约 75% 的持仓成本在 329.00 元以上，深套筹码密集",
    "判断 1.6T 光模块价格将维持在 700 美元以上，800G 产品 2027 年仅个位数降幅",
    "培育钻石价格三年跌超九成、均价由 2022 约 574 元/克拉跌到 151 元以下",
  ]) expect(posHits(s)).toEqual([]);
});

test("守卫：条件句 / 操作结论压过现状豁免（「如果…（9-17 收盘 X 元）」照样报）", () => {
  for (const s of [
    "如果股价在 8-07 筹码 15% 分位成本 910.20 元下方持续运行（9-17 收盘 896.00 元），那么减仓",
    "若当前股价回到 8-07 筹码 85% 分位成本 14.60 元之上则加仓",
  ]) expect(posHits(s).length).toBe(1);
});

test("守卫：「股价」「市价」「行情价格」不算产品价格主语", () => {
  for (const s of [
    "乐观情景下股价将维持在 58 元以上",
    "乐观情景下库内行情价格将回到 58 元以上",
    "乐观情景下市价将回到 58 元以上",
  ]) expect(posHits(s).length).toBe(1);
});

test("守卫：同句有商品每单位价，股价的位置式照样报（整句商品豁免要求紧挨动词前无「股」）", () => {
  expect(posHits("现货金 4,300 美元/盎司高位运行，股价在 58 元上方维持强势").length).toBe(1);
});
...
// 那会让每次全量跑都点亮一片假红线，真问题反而淹掉。但**只认标记**：目录空着照旧报未测。
