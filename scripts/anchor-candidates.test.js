...
// 2026-09-23 质检加了「位置式触发价位」一类。本脚本当时只认「具体」一种，
// 11 篇被拦、这里却报「✅ 当前没有被拦下的价位」——假绿。两类都要认。
test("位置式触发价位也算价位类硬红线", () => {
  expect(quotedPrice("【notes.md】位置式触发价位「回到筹码加权成本253元上方」（§4.9…）")).toBe("回到筹码加权成本253元上方");
});

test("位置式分诊：带锚→存量残留、裸价位→裸价位、区间→够不着（Stocks 导入存量原文）", () => {
  expect(classify("回到筹码加权成本253元上方").kind).toBe("stale");
  expect(classify("在  8-07 筹码 15% 分位成本 910.20 元 下方").kind).toBe("stale");
  expect(classify("在5.60元上方").kind).toBe("bare");
  expect(classify("站回 250.80 元之上").kind).toBe("bare");
  expect(classify("在筹码成本区12.7–13.6元上方").kind).toBe("unreachable");
});

test("位置式匹配从最左边的动词起：「→ 偏跌至」的第二个动词不当候选锚名", () => {
  expect(classify("→ 偏跌至 40 元下方").kind).toBe("bare");
});
