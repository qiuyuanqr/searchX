...
// 简报式结构（2026-08-26 晚改版）：一天一张卡、卡内多条条目。
// days：有序数组，元素 { entries: [{ type }] }。filters：{ type:'all'|<类型> }。
// 返回 { dayVisible:boolean[], entryVisible:boolean[][], count }；
// 天卡在其条目全部被筛掉时整卡隐藏，count 为可见条目总数。
export function computeDigestView(days, { type = "all" } = {}) {
  const dayVisible = new Array(days.length).fill(false);
  const entryVisible = [];
  let count = 0;

  for (let i = 0; i < days.length; i++) {
    const entries = days[i].entries || [];
    const vis = entries.map((e) => type === "all" || e.type === type);
    entryVisible.push(vis);
    const n = vis.filter(Boolean).length;
    count += n;
    dayVisible[i] = n > 0;
  }

  return { dayVisible, entryVisible, count };
}
