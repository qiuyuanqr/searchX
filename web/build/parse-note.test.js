...
test("「## 一句话讲清结论」这类标题不该被排除误伤（带「结论」即豁免）", () => {
  const raw = "---\ntype: 股票\n---\n\n# T\n\n## 一句话讲清结论\n\n真结论。\n";
  expect(parseNote(raw, "2026-07-31_x").tldr).toBe("真结论。");
});

test("整篇只有科普体裁标题、没有别的结论位时，兜底仍取它（导语不该整个空掉）", () => {
  const raw = "---\ntype: 股票\n---\n\n# T\n\n## 一句话讲清\n\n这家公司做逆变器。\n";
  expect(parseNote(raw, "2026-07-31_y").tldr).toBe("这家公司做逆变器。");
});

test("同篇另有「## 核心结论」时，科普节仍不得截胡（兜底只在没别的可用时生效）", () => {
  const raw =
    "---\ntype: 股票\n---\n\n# T\n\n## 核心结论（未来 3 个月）\n\n方向偏跌，估值透支。\n\n## 一句话讲清\n\n这家公司做逆变器。\n";
