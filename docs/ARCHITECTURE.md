...
| 墙内探活 | `services/runner/src/probe-cli.js` | 每个 runner tick（`scheduled-run.sh`） | 站点+Worker 主备端点；只有本机测得到墙内 SNI 阻断；连续断满 4 tick 才报警（瞬时抖动只留痕），限频 6h/封；失败类报警的正文由 `classifyFailure` 从本轮日志识别错误类型与处置办法，类型进主题 |
...
11. **改完再审一遍自己的改动**：2026-07-31 那轮修完 68 个已核验缺陷、测试全绿并上线，第二轮复审同一批 diff 发现 **15 个是修复本身引入的新问题**。动了锁 / 状态落盘 / 校验这类「出错就静默」的代码，收尾必须再过一轮，立场是推翻「已经修好了」。加一层保护前先问「是不是拆掉了另一层」——给锁加「运行期间定期更新时间戳」修掉了「长批次被误判超时」，同时也让「进程活着但卡死」永远不再被判超时。
12. **测试夹具绿不等于真实数据对**：本项目最主要的缺陷形态。改 `web/build/`、`report-to-obsidian.js`、查重逻辑后，必须写临时脚本对 `research/` 全部存量真跑一遍（脚本写系统临时目录，输出路径显式传绝对路径）。界面改动必须在真浏览器里把正常与异常两条路径都走一遍——纯函数测试测不到接线。
13. **shell 脚本兼顾 macOS 与 Linux**：本机 macOS、CI Linux，`stat` 取修改时间的参数不同（`-f %m` / `-c %Y`），且 GNU 的 `-f` 不报错、打印挂载点，`|| 兜底` 拦不住。解析 git 输出的文件名一律 `-c core.quotePath=false`，否则中文路径变八进制转义串、按文件名的判断全部失效（隐私红线上的机密文件名匹配就这样失效过）。锁文件与临时目录用固定路径 + 环境变量可覆盖，别用 `$TMPDIR`（ssh 与 launchd 拿到的值不同，互斥会形同虚设）。
