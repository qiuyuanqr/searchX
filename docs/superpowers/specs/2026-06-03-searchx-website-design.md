...
核心原则：**分阶段、逐步推进**——每一期都能独立上线、独立产生价值；不在一开始就搭建复杂的底层设施。
...
**非目标（本期不做）**：云端常驻无人值守 runner（B 模块，见 §11）；自然语言语义检索的完整实现（先做基础版，见 §5.4）；多用户账号体系；评论/社交。

## 3. 总体架构

关键边界：**对外公开的全是静态文件，没有可被攻击搞垮的服务器；唯一能对外写入的入口，是一段只负责把请求放进队列、不含其他逻辑的最小函数；任何花钱或有风险的动作，都必须等作者人工审批通过后才会执行。**
...
```
  ...
  └───────────────────────────────────────────┬───────────────┘
                                               │ (Turnstile+限制提交频率+长度校验)
                                               ▼
                                    Cloudflare Worker（只做入队的最小入口）
                                               │ 用受限 token 建 Issue
  ─────────────────────────────────────────────┼──────────────────────────
...
- 结果可复现：同样的 `research/` 输入，永远产出同样的 `site/`。
- 备选：若以后维护变复杂，可换 Eleventy 等轻量 SSG；接口不变。

### 5.3 信息流 UI（视觉与交互——已定稿）
定稿原型：`.superpowers/brainstorm/sess/content/feed-paper-v4.html`。
- **视觉风格**：仿纸张质感（暖色纸底、衬线字体、大量留白）——暖纸底 + 衬线正文，沿用 `report.html` 调色并**整体调淡**；支持深色模式。
- **结构**：顶部（小标 → 站名 → 搜索框 → 类型/板块筛选条）随整页一起滚动；下方为卡片信息流。
- **分隔**：无硬边框；卡片仅极淡阴影做底纹；条目间用**极细的渐变分隔线**，**留白主导**。
- **交互**：卡片 3D 倾斜跟随鼠标（"微微晃动"，≈4°）+ 悬浮上浮 + 标题上移并变成印章红色 + 左侧红书签条 + 按下时缩一下、松开弹回 + 卡片逐个依次淡入出现；**整页滚动 + "回到顶部"按钮**（滑过首条 1/3 出现）。
- **筛选**：按 `type`（概念/人物/方法论/事件/板块）与板块标签客户端过滤 `feed-index.json`。
- **无障碍**：尊重 `prefers-reduced-motion`（关闭倾斜/动画）。

### 5.4 搜索
- **基础版**：Pagefind（纯静态全文检索，零后端），用其 JS API + 仿纸张质感的自定义结果样式（参考站 ai-digest.liziran.com 即此方案）。
...
- 提交 POST 到 **Cloudflare Worker**（免费、无状态、**只入队、绝不触发花钱动作**）。Worker：校验 Cloudflare **Turnstile**（挡机器人）→ 限制提交频率（每 IP/邮箱每日上限）→ 输入长度上限/清洗 → 用**受限 GitHub token**（仅能在该仓库建 Issue）创建一条 Issue（标签 `pending`）。

### 6.2 队列与通知
- **GitHub Issues** 即队列；标签做状态机：`pending → approved / rejected → done`。
- 通知走 **GitHub 原生**（建 Issue 即邮件提醒作者），零额外搭建。

### 6.3 人工审批环节（approve-before-spend）
- **通知**：Worker 建 Issue 时 **@ 作者（或指派给作者）**，确保 GitHub 自动发邮件到作者邮箱（题目即邮件标题）。作者在**手机/电脑**上看 Issue 即可审批，**无需在 Mac 前**。
- 作者看 Issue（只是题目 + 谁提的，**不是报告**），决定 👍 / 👎。
- 同意 = 给 Issue 贴 **`approved` 标签**（**已敲定**；评论 `approve` 作为可选备选）。**额度只从这一刻起才消耗。** 驳回 = 关 Issue，不花钱、也不发布任何内容。
...
- **不是常驻守护进程，而是「一键启动」**：作者审批后回到 Mac，跑一条命令 `bun run runner`。它：
  ...
  2. 逐条把题目交给本机 **Claude Code 跑 `/research`**（**轻量研究模式**，见 §6.7）；
  3. 产出三件套写入 `research/`；
  4. commit + push（**复用 SKILL Step 6 的自动上线**）；
  5. 给 Issue 贴 `done`；
  6. 触发 Emailer（§7）。
- **Claude 额度全程不出本机**；研究只在作者开机跑这条命令时发生（"先审后跑"场景足够）。
- 失败/中断要可重入：以 Issue 的 `done` 标签作为幂等标记（同一条重复跑也不会重复处理），重跑只处理未完成的。

### 6.5 自动发布（✅ 已实现 2026-06-03）
- Runner 的 push 触发 §5.5 的 Action：自动构建 + 部署，新条目自动出现在信息流。
- **`/research` SKILL 已加 Step 6**：构建检查（`bun test && bun run web/build/cli.js`，必须全部通过才能推送上线）→ 隐私最终检查（推送前最后一遍）→ 精准 `git add 本主题文件夹 + INDEX` → push main → Pages 自动部署。验证：单篇推送后约 16s 线上 200。Runner 直接复用，无需另写发布逻辑。

### 6.6 安全模型（纵深防御，即层层设防、互为后备）
- 公开面是静态文件（CDN 托管，无服务器可崩）。
- 唯一公开写入口（Worker）：Turnstile + 限制提交频率 + 长度上限 + 可选"朋友口令/邮箱白名单"。
- **最强一道：人工审批在花钱之前。** 恶意大量提交的最坏后果只是待审列表多几条，一键驳回。
- 保底：全局**预算硬上限** + 队列长度上限。
- 密钥卫生：Claude API key 永不进任何公开前端；Worker 只持有"仅能建 Issue"的受限 GitHub token。

### 6.7 Token 模型与轻量研究模式（已敲定 · 省 token）

> **🔄 2026-06-03 修正（用户决定）：取消「轻量研究模式」——所有朋友的请求一律按完整深度来做，Runner 不再附加任何用于缩减研究规模的标记。本节下文及 §10.7 关于「轻量研究模式 / 缩减研究规模」的内容作废；成本由「人工审批环节」逐条把控（每条都是完整 `/research`）。**

- **唯一消耗 Claude 调用额度的地方 = 跑一次 `/research` 本身。** 其余全是行为固定、不调用 AI（不消耗 token）的脚本：
  - 网站/信息流页面/搜索索引 = `web/build/`（bun）按固定规则套模板生成，**零 token**（M1 已如此）；
  - 报告外壳 `templates/report.html` = 固定文件，纯字符串替换，**零 token**；
  - 报告**内容**由大模型在 `/research` 时**生成一次**，之后永远是静态文件被复用，不会反复消耗额度。
- **朋友请求默认走「轻量研究模式」**：相对作者自用的「完整深度」减少并行子代理数、降低检索轮次、跳过对抗式交叉验证那一层，单条成本明显更低。作者可对个别题目手动升级为完整深度。
- **预算保底**：队列长度上限 + 可选全局硬上限；人工审批已是花钱前最强的一道把关。
- 实现提示：轻量研究模式可通过 Runner 给 `/research` 传一个"轻量"信号（如 Issue 题目后缀或 runner 参数），SKILL 据此缩减检索和子代理规模——具体在写 plan 时定。

## 7. 第 3 期 · 邮件触达
- 研究 `done` 后，Emailer 给提交者发一封**易读邮件**：核心结论摘要（TLDR + 关键发现）+ 网站链接（可选 PDF 附件），抄送作者。
- 发送方式：先用**已连接的 Gmail**（低量够用）；量大再换专用发信服务（Resend/Postmark）。
- **零 token（已敲定）**：Emailer 是纯 bun 脚本，读**已生成**的 TLDR + 关键发现 + 网址塞进固定模板发出，**不调用大模型**。极简邮件（一句话摘要 + 网站链接，抄送作者）即可——体验好且基本零成本。所以按"只要消耗大模型额度就不做"这条规则衡量：发邮件不消耗额度，可以做。
...
## 9. 技术栈小结
- 站点：静态 HTML/CSS/JS（仿纸张质感主题，原生、无框架依赖）。
...
7. ~~**朋友请求默认走轻量研究模式**省 token~~ → **已撤销（2026-06-03，用户决定）：所有请求一律按完整深度来做，成本由人工审批环节把控。**
8. **发极简邮件**给提交者（纯脚本、不消耗 token），抄送作者。（2026-06-03 敲定）
9. 网站构建与邮件**均为行为固定、不调用 AI 的脚本，不消耗 token**；唯一消耗额度处 = 跑一次 `/research`。（2026-06-03 厘清）
10. **自动上线已实现**：SKILL Step 6（push main → Pages）。（2026-06-03 完成）

## 11. 推迟 / 未来
- **B 模块·云端 runner**：常驻服务/Action 用 Claude API 无人值守跑研究（要 API 计费 + 额度硬上限）。本机版跑顺后再升级，其它单元可复用。
- **自然语言/语义检索**：见 §5.4。
...
- **M2（第 2 期）**：拆成两段、各自独立可上线（M2 跨多子系统，故分解）：
  - **M2a · 提交入队流程**：提交表单（静态页，复用仿纸张质感主题）+ Cloudflare Worker（Turnstile + 限制提交频率 + 长度上限）+ 建 `pending` Issue + @作者。**验收**：站上提交 → 你邮箱收到通知 + 仓库出现 `pending` Issue，全程不花钱、也不发布任何内容。
  - **M2b · Runner**：`bun run runner` 取 `approved` 未 `done` 的 Issue → `/research` 轻量研究模式 → push（自动上线已做）→ 贴 `done` → 触发 Emailer。**验收**：贴 `approved` → 跑一条命令 → 报告自动上线 + 提交者收到邮件。
...
- **M2**：从站内表单提交一个题目 → 作者收到通知 → 贴 `approved` → 本机自动跑出报告并 push → 站点自动更新出现该条目；驳回路径不花钱。
